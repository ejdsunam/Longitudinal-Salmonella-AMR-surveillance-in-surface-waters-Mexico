---
title: "AMR and Salmonella Dynamics Pipeline"
author: "Enrique J. Delgado Suárez"
...
```{r glmm_prevalence, message=FALSE, warning=FALSE}
...
```{r glmm_mdrf, message=FALSE, warning=FALSE}
...
```{r run_continuous_trend_function}
...
```{r execute_batch_continuous_trends}
...
```{r export_continuous_trends_csv}
...
```{r run_targeted_year_glm_function}
...
```{r execute_batch_genotypic_trends}
...
```{r prepare_jaccard_distance_matrix}
...
```{r format_temporal_permdisp_label}
...
```{r export_temporal_permdisp_csv}
...
```{r plot_temporal_permdisp_boxplot, message=FALSE, warning=FALSE}
...
```{r calculate_regional_permutest}
...
```{r extract_regional_anova_metrics}
...
```{r format_regional_permdisp_label}
...
```{r export_regional_permdisp_csv}
...
```{r plot_regional_permdisp_boxplot, message=FALSE, warning=FALSE}
...
```{r extract_nmds_dynamic_subtitles, message=FALSE, warning=FALSE}
...
```{r plot_temporal_nmds_figure, message=FALSE, warning=FALSE}
...
```{r calculate_global_cohen_kappa}
...
```{r extract_kappa_matrix_metrics}
...
```{r build_dynamic_alluvial_subtitle_string}
...
```{r plot_longitudinal_alluvial_flow}
...
```{r run_prevalence_validation_glm}
...
```{r extract_prevalence_glm_metrics}
...
```{r build_prevalence_validation_dataframe}
...
```{r print_prevalence_validation_matrix}
...
```{r export_prevalence_validation_log}
...
```{r raw_mdrf_isolate_audit_setup}
...
```{r build_mdrf_validation_dataframe}
...
```{r print_mdrf_validation_matrix}
